    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="glass-card rounded-xl p-3 sm:p-4 space-y-3">
        ...
        <div className="flex gap-1 sm:gap-1.5 flex-wrap">
          {(Object.keys(filterLabels) as DateFilter[]).map((f) => (
            <button key={f} onClick={() => setDateFilter(f)}
              className={`px-2.5 sm:px-3 py-1.5 rounded-lg text-[10px] sm:text-xs font-medium transition-all ${dateFilter === f ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>
              {filterLabels[f]}
            </button>
          ))}
        </div>
        {dateFilter === "custom" && (
          <div className="flex gap-2">
            <input type="date" value={customStart} onChange={(e) => setCustomStart(e.target.value)} className="flex-1 px-2 sm:px-3 py-2 rounded-lg bg-secondary text-foreground text-xs border border-border" />
            <input type="date" value={customEnd} onChange={(e) => setCustomEnd(e.target.value)} className="flex-1 px-2 sm:px-3 py-2 rounded-lg bg-secondary text-foreground text-xs border border-border" />
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        <div className="glass-card rounded-xl p-3 sm:p-4 text-center">
          <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-green-400 mx-auto mb-1" />
          <p className="text-[10px] sm:text-xs text-muted-foreground">Receita</p>
          <p className="text-sm sm:text-lg font-bold text-green-400">R$ {revenue.toFixed(0)}</p>
        </div>
        <div className="glass-card rounded-xl p-3 sm:p-4 text-center">
          <TrendingDown className="w-4 h-4 sm:w-5 sm:h-5 text-red-400 mx-auto mb-1" />
          <p className="text-[10px] sm:text-xs text-muted-foreground">Despesas</p>
          <p className="text-sm sm:text-lg font-bold text-red-400">R$ {totalExpenses.toFixed(0)}</p>
        </div>
        <div className="glass-card rounded-xl p-3 sm:p-4 text-center">
          <DollarSign className="w-4 h-4 sm:w-5 sm:h-5 text-primary mx-auto mb-1" />
          <p className="text-[10px] sm:text-xs text-muted-foreground">Lucro</p>
          <p className={`text-sm sm:text-lg font-bold ${profit >= 0 ? "text-green-400" : "text-red-400"}`}>R$ {profit.toFixed(0)}</p>
        </div>
      </div>
          ...
          <div className="h-40 sm:h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <XAxis dataKey="name" tick={{ fill: "hsl(220 10% 55%)", fontSize: 9 }} />
                <YAxis tick={{ fill: "hsl(220 10% 55%)", fontSize: 9 }} width={40} />
                <Tooltip contentStyle={{ backgroundColor: "hsl(220 14% 11%)", border: "1px solid hsl(220 12% 18%)", borderRadius: 8, color: "hsl(40 20% 92%)", fontSize: 12 }} />
